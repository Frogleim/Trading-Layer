import os
import requests
import zipfile
import pandas as pd
from io import BytesIO

BASE_URL = "https://data.binance.vision/data/futures/um/monthly/bookTicker"


def download_monthly_klines(symbol, interval="3m", years=[2024, 2025], months=range(1,13), days=range(1, 31)):
    out_path = f"./downloads/{symbol}"
    os.makedirs(out_path, exist_ok=True)  # do NOT wrap in try/except

    for year in years:
        for month in months:
            mm = f"{month:02d}"
            file_name = f"{symbol}-bookTicker-{year}-{mm}.zip"
            csv_name  = f"{symbol}-bookTicker-{year}-{mm}.csv"

            csv_path = os.path.join(out_path, csv_name)

            if os.path.exists(csv_path):
                print(f"⏭️ Already exists, skipping: {csv_name}")
                continue

            url = f"{BASE_URL}/{symbol}/{file_name}"
            print(url)
            print(f"⬇️ Downloading {file_name} ...")

            r = requests.get(url)
            if r.status_code != 200:
                print(f"❌ Not found: {file_name}")
                continue

            z = zipfile.ZipFile(BytesIO(r.content))
            z.extract(csv_name, out_path)

            print(f"📁 Extracted {csv_name}")
if __name__ == '__main__':
    from main import get_all_tradable_symbols

    # symbols = get_all_tradable_symbols()
    symbols = ["1000BONKUSDT"]
    symbols = sorted(symbols)
    for sym in symbols:
        download_monthly_klines(sym)   # ✔ one symbol at a time